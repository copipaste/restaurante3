            ...
            <x-adminlte-datatable id="table1" :heads="$heads" striped head-theme="white" hoverable with-buttons>
                ...
                @foreach ($categorias as $categoria)
                    <tr>
                        ...
                        <td>
                            <span class="{{ ($categoria->status == 1 ) ? 'bg-primary' : 'bg-secondary' }} badge rounded-pill">{{ ($categoria->status == 1 ) ? 'Activo' : 'Inactivo' }}</span>
                            {{-- <span class="badge rounded-pill ">Secondary</span> --}}
                        </td>

                        <td>
                            {{-- <img src="{{asset('storage').'/'.$categoria->url}}" alt="" width="200px" class="img-thumbnail"> --}}
                            <img src="{{asset($categoria->url)}}" alt="{{$categoria->title}}" width="100px" class="img-fluid img-thumbnail">
                        </td>
                        ...
                        <td width="15px"> {{-- esto es como una columna mas  --}}
                            <div class="d-flex"> {{-- esto es lo que hace que los datos esten uno al lado del otro --}}
                                ...
                                <button class="btn btn-xs btn-default text-primary mx-1 shadow" title="edit" data-toggle="modal" data-target="#modalEditarCategoria{{ $categoria->id }}">
                                    <i class="fa fa-lg fa-fw fa-pen"></i>
                                </button>


                                {{-- MODAL EDITAR--}}
                                <x-adminlte-modal id="modalEditarCategoria{{ $categoria->id }}" title="Editar Promocion" size="lg" theme="dark" icon="fas fa-bell" v-centered static-backdrop scrollable>    
                                                         ...
                                                         <div class="form-group">
                                                          ...
                                                          <div class="form-group">
                                                             ...
                                                             <select name="status" id="status" class="form-control">
                                                                 ...
                                                                 <option value="1">Activo</option>
                                                                 <option value="0">Inactivo</option>
                                                                 <option value="1" {{ $categoria->status == '1' ? 'selected' : '' }}>Masculino</option>
                                                                 <option value="0" {{ $categoria->status == '0' ? 'selected' : '' }}>Femenino</option>
                                                             </select>
                                                          </div>
                                                          <div class="form-group">
                                                             <label for="url">Imagen</label>
                                ...
                                {{-- boton de eliminar  --}}
                                <button class="btn btn-xs btn-default text-danger mx-1 shadow" title="Delete" data-toggle="modal" data-target="#modalCustom{{ $categoria->id }}">
                                    <i class="fa fa-lg fa-fw fa-trash"></i>
                                </button>
                                {{-- MODAL ELIMINAR--}}
                                <x-adminlte-modal id="modalCustom{{ $categoria->id }}" title="Eliminar" size="sm" theme="dark"
